    ...
    for gkind in kinds:
        for group in conn.execute(
            ...
            members = conn.execute(
                """
                SELECT p.photo_id, p.rel_path FROM group_members gm
            ...
            canvas = Image.new(
                "RGB", (cols * cell_size, rows * (cell_size + label_h)), "white"
            )
            ...
            for idx, member in enumerate(members):
                ...
                img = None
                thumb = thumbs_dir / f"{member['photo_id']}.jpg"
                if thumb.exists():
                    try:
                ...
                label = _fit_label(
                    draw, Path(member["rel_path"]).name, font, cell_size - 8
                )
                draw.text((x0 + 4, y1 + 2), label, fill="black", font=font)

            path = out_dir / f"{gkind}_G{group['group_id']:03d}.jpg"
